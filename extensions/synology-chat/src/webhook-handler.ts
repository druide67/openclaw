...
export function createWebhookHandler(deps: WebhookHandlerDeps) {
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      // deliver() dispatches the reply via the SDK's buffered block dispatcher
      // (in channel.ts). It always returns null — actual sending happens inside
      // the dispatcher's deliver callback, not via this return value.
      await deliver({
        body: cleanText,
