            ...
            await rt.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
              ...
              dispatcherOptions: {
                deliver: async (payload: { text?: string; mediaUrl?: string }) => {
                  ...
                  if (!text) {
                    log?.info?.(`deliver: empty text payload, skipping`);
                    return;
                  }

                  log?.info?.(
                    `deliver: sending ${text.length} chars to ${isGroup ? `channel ${msg.channelId}` : `user ${sendUserId}`}`,
                  );
                  ...
                  if (isGroup && msg.channelId) {
                    ...
                    if (channelWebhookUrl) {
                      await sendToChannel(channelWebhookUrl, text, account.allowInsecureSsl, log);
                    } else {
                      log?.warn?.(
                ...
                onError: (err: unknown, info: { kind: string }) => {
                  log?.error?.(
                    `Dispatcher error (${info.kind}): ${err instanceof Error ? err.message : String(err)}`,
                  );
                },
                onSkip: (payload: unknown, info: { kind: string; reason: string }) => {
                  log?.info?.(`Dispatcher skipped ${info.kind}: ${info.reason}`);
                },
              },
            });
